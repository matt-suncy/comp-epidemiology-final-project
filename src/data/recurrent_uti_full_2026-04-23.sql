WITH uti_concepts AS (
    ...
    WHERE ancestor_concept_id = 81902
     ...
     abx_0_10d_first AS (
         SELECT
             person_id,
             drug_exposure_start_date AS first_abx_0_10d_date,
             drug_concept_id AS first_abx_0_10d_concept_id
         FROM (
                  SELECT
                      person_id,
                      ...
                      ROW_NUMBER() OVER (
                          PARTITION BY person_id
                          ORDER BY drug_exposure_start_date ASC, drug_concept_id ASC
                          ) AS row_num
                  FROM abx_0_10d_all
              ) x
         WHERE row_num = 1
     ...
     prior_uti_history AS (
         SELECT
             ...
             MAX(u.uti_date) AS last_uti_date_1yr
         FROM index_uti i
     ...
     prior_abx_history AS (
         SELECT
             ...
             MAX(de.drug_exposure_start_date) AS last_abx_date_1yr
         FROM index_uti i
                  JOIN drug_exposure de
                       ON i.person_id = de.person_id
                           AND de.drug_concept_id IN (SELECT concept_id FROM abx_concepts)
...
SELECT
    ...
    d.age_at_index,
    d.gender_concept_id,
    CASE WHEN d.age_at_index >= 65 THEN 1 ELSE 0 END AS age_65_plus_flag,
    CASE WHEN d.age_at_index >= 80 THEN 1 ELSE 0 END AS age_80_plus_flag,

    CASE WHEN r.person_id IS NOT NULL THEN 1 ELSE 0 END AS recurrent_uti_90d_flag,
    r.first_recurrent_uti_date,

    af.first_abx_0_10d_date,
    af.first_abx_0_10d_concept_id,
    CASE WHEN af.person_id IS NOT NULL THEN 1 ELSE 0 END AS any_abx_0_10d_flag,
    COALESCE(asum.num_distinct_abx_0_10d, 0) AS num_distinct_abx_0_10d,
    CASE WHEN COALESCE(asum.num_distinct_abx_0_10d, 0) > 1 THEN 1 ELSE 0 END AS abx_switch_0_10d_flag,

    CASE WHEN ra.person_id IS NOT NULL THEN 1 ELSE 0 END AS repeat_abx_11_30d_flag,
    ra.first_repeat_abx_date,
    CASE
        WHEN ra.first_repeat_abx_date IS NOT NULL
            THEN DATEDIFF(day, i.uti_date, ra.first_repeat_abx_date)
        ELSE NULL
        END AS days_to_repeat_abx,

    CASE WHEN e.person_id IS NOT NULL THEN 1 ELSE 0 END AS er_30d_flag,
    e.first_er_date,
    CASE
        WHEN e.first_er_date IS NOT NULL
            THEN DATEDIFF(day, i.uti_date, e.first_er_date)
        ELSE NULL
        END AS days_to_er_visit,
    ...
    CASE WHEN COALESCE(ph.prior_uti_count_1yr, 0) > 0 THEN 1 ELSE 0 END AS prior_uti_any_1yr_flag,
    CASE WHEN COALESCE(ph.prior_uti_count_1yr, 0) >= 2 THEN 1 ELSE 0 END AS recurrent_history_1yr_flag,
    ph.last_uti_date_1yr,
    CASE
        WHEN ph.last_uti_date_1yr IS NOT NULL
            THEN DATEDIFF(day, ph.last_uti_date_1yr, i.uti_date)
        ELSE NULL
        END AS days_since_last_uti,
    ...
    CASE WHEN COALESCE(pa.prior_abx_exposure_count_1yr, 0) > 0 THEN 1 ELSE 0 END AS prior_abx_any_1yr_flag,
    pa.last_abx_date_1yr,
    CASE
        WHEN pa.last_abx_date_1yr IS NOT NULL
            THEN DATEDIFF(day, pa.last_abx_date_1yr, i.uti_date)
        ELSE NULL
        END AS days_since_last_abx,
    ...
    COALESCE(cf.ckd_ever_flag, 0)
        + COALESCE(hf.htn_ever_flag, 0)
        + COALESCE(kff.kidney_failure_ever_flag, 0)
        + COALESCE(df.diabetes_ever_flag, 0) AS comorbidity_count_ever
...
FROM index_uti i
         ...
         LEFT JOIN abx_0_10d_first af
                   ON i.person_id = af.person_id
